...
export default function ScheduleInfo() {
  ...
  const {data: loadSchedule, isLoading}: any = useQuery('loadSchedule', () =>
    pendingSchedule(tenant)
  )
  ...
  const {mutate: completeSchedule} = useMutation(patchSchedule, {
    onSuccess: () => {
      ...
      const equipmentHours = allHours?.data.filter(
        (hour: any) => hour.fleetId === form.getFieldValue('fleetId')
      )
      mutateHours({
        ...
        previousReading:
          equipmentHours?.length > 0
            ? equipmentHours[equipmentHours.length - 1]?.currentReading
            : 0,
        currentReading: form.getFieldValue('currentReading')
          ? form.getFieldValue('currentReading')
          : equipmentHours[equipmentHours.length - 1]?.currentReading,
        tenantId: tenant,
        date: form.getFieldValue('completedDate')
          ? form.getFieldValue('completedDate')
          : new Date(),
  ...
  const handleCompleteBacklog = (entryId: any, fleetId: any) => {
    ...
    console.log(
      'allHours',
      allHours?.data?.filter((hour: any) => hour.fleetId?.trim() === fleetId?.trim())
    )
    const latestReading = allHours?.data?.filter(
      (hour: any) => hour.fleetId?.trim() === fleetId?.trim()
    )[0]?.currentReading
  ...
  const columns: any = [
    ...
    },
    {
      ...
      render: (serviceTypeId: any) => {
        return serviceType?.data.find((service: any) => service.id === serviceTypeId)?.name
      },
    },
    {
      title: 'Action',
        ...
        return (
          <Space size='middle'>
            ...
            <button
              ...
              onClick={() => handleCompleteBacklog(entryId, record?.fleetId)}
            >
              Complete
            </button>
            <button
              ...
              onClick={() => navigate(`/entries/backlog/${record?.fleetId}`)}
            >
              View Backlogs
            </button>
          </Space>
