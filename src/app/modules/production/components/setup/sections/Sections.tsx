...
const SectionsPage = () => {
  ...
  const {data: allServices} = useQuery('services', () => fetchServices(tenant), {
    cacheTime: 60000000,
  })
  ...
  useEffect(() => {
    const getModel = () => {
      let newName
      newName = allServices?.data?.find((item: any) => item.id.toString() === params.id)
      return newName
    }
    ...
    const getItemName = async (param: any) => {
      ...
      const itemTest = await allServices?.data.find((item: any) => item.id.toString() === param)
