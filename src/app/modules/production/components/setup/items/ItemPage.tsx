...
const ItemsPage = () => {
  ...
  const getModel = () => {
    let newName = null
    const itemTest = allGroups?.data.find((item: any) => item.id.toString() === params.id)
    newName = itemTest
    return newName
  }
  ...
  const getItemName = async (param: any) => {
    let newName = null
    const itemTest = await allGroups?.data.find((item: any) => item.id.toString() === param)
  ...
  const onFinish = async (values: any) => {
    ...
    console.log(data)
    try {
      const response: any = await axios.post(url, data).then((response) => {
        ...
        try {
          axios.post(`${ENP_URL}/ItemValue`, data).then((response) => {
