...
const Sequence = () => {
  ...
  const {
    ...
    ['sequences'],
    () => getSequences(routeParams?.modelId?.replaceAll('/', '2%F'), tenant),
    {
      retry: false,
    }
  )
  const {mutate: removeSequence, isLoading: isDeleteLoading} = useMutation(
    (model: any) => deleteSequence(tenant, model),
    {
      onSuccess: () => {
        queryClient
          .refetchQueries(['sequences'])
          .then(message.success('Sequence deleted successfully'))
      },
      onError: (error: any) => {
