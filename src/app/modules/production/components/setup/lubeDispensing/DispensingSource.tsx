...
import {
  addSources,
  deleteDispensingSource,
  deleteSources,
  getDispensingSources,
  getSources,
  postDispensingSource,
  putSources,
} from '../../../../../urls'
import {useAuth} from '../../../../auth'
import React, {useState} from 'react'

const DispensingSource = () => {
  const {tenant} = useAuth()
  const queryClient: any = useQueryClient()
  const {data: sources, isLoading} = useQuery('dispensingSource', () =>
    getDispensingSources(tenant)
  )
  const {mutate: addSource} = useMutation((data) => postDispensingSource(data, tenant), {
    onSuccess: () => {
      message.success('Source added successfully')
      queryClient.invalidateQueries('dispensingSource')
      form.resetFields()
  ...
  const {mutate: updateSource} = useMutation(putSources, {
    onSuccess: () => {
  ...
  const {mutate: removeSource} = useMutation(deleteDispensingSource, {
    onSuccess: () => {
  ...
  return (
    <KTCard>
      <KTCardBody>
        ...
        <Table columns={columns} bordered dataSource={sources?.data} loading={isLoading} />
        <Modal
...
export default DispensingSource
