...
const HoursModelClass: any = () => {
    ...
    const {data: defaultData, isLoading} = useQuery('all-equipment', () => getEquipment(tenant))
    ...
    const {mutate: editHourReading, isLoading: isEditLoading} = useMutation(editHour, {
        onSuccess: () => {
            ...
            queryClient.invalidateQueries('all-hours').then(r => r)
            queryClient.invalidateQueries('all-equipment').then(r => r)
    ...
    const [editableKeys, setEditableRowKeys] =
      useState<React.Key[]>(() => defaultData?.data?.map((item: any) => item.equipmentId?.trim()));
            ...
            render: (text, record, _, action) => {
                return [
                    <button
                      ...
                      type='button'
                      className={
                          record?.id ? 'btn btn-primary' : 'btn btn-danger disabled'
                      }
                      onClick={() => {
                          handleEdit(record);
                      }}
                      ...
                      <ErrorBoundary>
                          <EditableProTable<DataSourceType>
                            ...
                            value={filterData(defaultData?.data?.map((item: any) => {
                                  const equipmentLatestHour = item?.hoursEntries?.length > 0 ? item?.hoursEntries[0] : undefined
                            ...
                            editable={{
                                type: 'multiple',
                                editableKeys: editableKeys ? editableKeys : defaultData?.data?.map((item: any) => item.equipmentId?.trim()),
                                            ...
                                            validator(_, value) {
                                                if (!value) {
                                                    ...
                                                    if (value <= getFieldValue('previousReading')) {
                                                        return Promise.reject(`Reading should be more than ${getFieldValue('previousReading')}`)
                                                    } else if (value > getFieldValue('currentReading')) {
                                                        return Promise.reject(`Reading cannot be more than ${getFieldValue('currentReading')}`)
                                                    } else {
                                                        return Promise.resolve();
                                                    }
                                                }
