...
const EquipWithPendingSetup = () => {
  const columns: any = [
    {
      ...
      render: (code: any) => {
        return <Tag color='blue'>{code?.toUpperCase()}</Tag>
      },
    },
    {
      title: 'Manufacturer',
      ...
      render: (manufacturer: any) => {
        return manufacturer?.name?.toUpperCase()
      },
    },
    {
      title: 'Action',
      ...
      render: (record: any) => (
        <Space size='middle'>
          <Link
            to={`/setup/service/${record?.code?.replaceAll('/', '%2F')}`}
            state={{
              ...record,
  ...
  const {data: modelData, isLoading: modelLoading} = useQuery('models', () =>
    getModels(getTenant())
  )
  const {data: equipmentData, isLoading: equipmentLoading} = useQuery('equipments', () =>
