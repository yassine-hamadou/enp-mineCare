...
const Prediction = () => {
  ...
  const {data: equipmentData, isLoading: equipmentDataLoading} = useQuery('equipments', () =>
    getEquipment(getTenant())
  )
    ...
    const data = equipmentData?.data
      ? equipmentData?.data?.map((equipment: any) => {
          return {
            ...equipment,
            latestHourFromPM:
              getEquipmentHoursEntry2?.data?.filter((equipmentHour: any) => {
  ...
  const {data: getEquipmentHoursEntry2, isLoading: equipHourLoading} = useQuery(
    'getEquipmentLatestHour',
    () => getAllEquipmentsHoursEntryPmReading(tenant)
  )
    ...
    // },
    {
      ...
      render: (record: any) => {
        ...
        const pmObjects = getPMObjects(
          record?.hoursEntries[0]?.currentReading,
          record?.model?.services?.map((service: any) => {
            return {
              name: service?.name,
              hours: service?.intervalForPm,
            }
