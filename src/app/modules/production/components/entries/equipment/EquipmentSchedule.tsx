...
function EquipmentSchedule() {
  ...
  const {data: serviceTypes, isLoading: servicesLoading} = useQuery('serviceTypes', () =>
    fetchServiceTypes(tenant)
  )
  ...
  const columns: any = [
    ...
    },
    {
      ...
      render: (serviceTypeId: any) => {
        return serviceTypes?.data.find((service: any) => service.id === serviceTypeId)?.name
      },
    },
    // {
    //   title: 'Action',
    ...
    >
      <Tabs
        ...
        items={[
          ...
          {
            label: (
              <Badge count={numberOfModelsPendingSetup ? numberOfModelsPendingSetup : 0}>
                <span className='me-4'>Schedule Parameters</span>
              </Badge>
            ),
            key: '3',
            children: (
