...
const FaultTable = () => {
  ...
  const handleInputChange = (e: any) => {
    ...
    if (e.target.value === "") {
      ...
      setGridData(initialData?.data.filter((fault: any) => fault.status === 0)
  ...
  const globalSearch = (searchValue: string) => { //searchValue is the value of the search input
    let initialData: any = QueryClient.getQueryData("faults");
    const searchResult = initialData?.data.filter((item: any) => {
      return (
